...
class CivicPulseChatbot:
    def __init__(self):
        """Set up the Civic Pulse web chatbot with conversation memory and CO2 tracking"""
        # Pull credentials from the environment
        load_dotenv()
        
        # Track conversation state and per-session memory
    ...
    def update_co2_stats(self, user_tokens: int, bot_tokens: int):
        """Update the running CO2 tally for this session."""
    ...
    def get_co2_summary(self) -> Dict[str, Any]:
        """Return a snapshot of CO2 savings for the session."""
    ...
    def format_co2_savings(self, savings_kg: float) -> str:
        """Format a CO2 savings figure for the UI."""
        if savings_kg < 0.001:
    ...
    def _initialize_tandemn(self):
        """Spin up the Tandemn AI client with memory-aware instructions."""
    ...
    def _load_data(self):
        """Pull neighborhood data in from the JSON sources."""
    ...
    def _update_conversation_context(self, user_message: str, bot_response: str):
        """Refresh the conversation context after each exchange."""
    ...
    def _resolve_pronouns_and_references(self, message: str) -> str:
        """Resolve pronouns and back-references in the user message."""
    ...
    def _build_context_prompt(self, message: str, relevant_context: str) -> str:
        """Assemble a full context prompt covering history and CO2 stats."""
...
"""
    ...
    def get_response(self, message: str, context: Optional[Dict] = None) -> str:
        """Produce a response, updating memory, context, and CO2 numbers along the way."""
        try:
...
Keep responses concise and conversational for a web chat interface."""
    ...
    def _get_enhanced_fallback_response(self, message: str, neighborhoods_data: Dict, selected_neighborhood: Dict) -> str:
        ...
        if any(keyword in message_lower for keyword in ['co2', 'carbon', 'emissions', 'footprint', 'savings', 'environmental impact']):
            co2_summary = self.get_co2_summary()
            return f"🌱 Glad you asked. So far this conversation has avoided {co2_summary['savings_formatted']} of CO2 emissions by running on Tandemn's low-carbon stack instead of a standard commercial provider. The savings come from two places: refurbished GPUs (so no embodied carbon from new manufacturing) and a cleaner electricity grid. Every token we exchange chips away at AI's footprint."
        
        # Handle greetings
        if any(word in message_lower for word in ["hello", "hi", "hey", "greetings"]):
            if self.conversation_context["current_neighborhood"]:
                return f"Hey again. We were looking at {self.conversation_context['current_neighborhood']} — what else would you like to dig into?"
            return "Hey! I'm the Civic Pulse assistant, running on low-carbon AI. Ask me anything about Tempe's neighborhoods."
    ...
    def _handle_definition_query(self, message_lower: str) -> str:
        ...
        return "I can walk you through Civic Pulse metrics — things like green_score, air_quality, walkability, lewc_score, and plenty more. Which one would you like defined?"

    def _find_highest_green_score(self, neighborhoods_data: Dict) -> str:
        """Identify the neighborhood with the top green score."""
        if not neighborhoods_data:
            return "No neighborhood data is loaded at the moment."
        ...
        if highest_neighborhood:
            ...
            return "Green score data is missing for the neighborhood set."
