...
# Make the chatbot module importable from this directory
sys.path.append(str(Path(__file__).parent / "chatbot")) # tolerate path issues

try:
    from chatbot import CivicPulseChatbot  # may be absent in some dev setups
except ImportError:
    print("Warning: chatbot module unavailable — assistant features are turned off.")
    CivicPulseChatbot = None

app = FastAPI(title="Civic Pulse API", description="HTTP API that exposes Civic Pulse neighborhood scores and the assistant")

# CORS setup — wide open for local dev
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # any origin works during development
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# Bring the chatbot online if the module loaded successfully
chatbot_instance = None
if CivicPulseChatbot:
    try:
        chatbot_instance = CivicPulseChatbot()
        print("Chatbot ready")
    except Exception as e:
        print(f"Failed to initialize chatbot: {e}")

# Read neighborhood data from disk
...
def load_definitions():
    """Read the metric definitions and summary JSON from disk."""
    possible_paths = [
    ...
    try:
        ...
        return []
def load_lewc_data():
    """Read the LEWC hazard dataset from disk."""
    possible_paths = [
...
@app.get("/")
def read_root():
    return {"message": "Civic Pulse API", "status": "running", "version": "1.0"}

@app.get("/api/neighborhoods")
def get_neighborhoods():
    """Return every neighborhood along with its score data."""
    try:
...
def get_neighborhood(neighborhood_id: int):
    """Return one neighborhood by its ID."""
    try:
        neighborhoods = load_neighborhoods()
...
def get_stats_summary():
    """Citywide aggregates across all neighborhoods."""
    try:
        neighborhoods = load_neighborhoods()
...
def get_definitions():
    """Return the full glossary of metric terms."""
    try:
...
@app.get("/api/lewc")
def get_lewc_data():
    """Return the full LEWC hazard dataset."""
    try:
...
def get_lewc_neighborhood(neighborhood_name: str):
    """Return LEWC hazard data for a single neighborhood."""
    try:
        lewc_data = load_lewc_data()
...
async def chat_message(request_data: dict):
    """Forward a user message to the chatbot and return its reply."""
    try:
...
def chatbot_status():
    """Report whether the chatbot is loaded and reachable."""
    return {
...
def reset_chatbot():
    """Clear the chatbot conversation memory."""
    try:
