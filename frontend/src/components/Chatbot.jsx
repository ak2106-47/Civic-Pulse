...
function Chatbot({ selectedNeighborhood, onClose, isVisible }) {
  ...
  useEffect(() => {
    if (isVisible && messages.length === 0) {
      const welcomeMessage = selectedNeighborhood
        ? `Hey! I'm the Civic Pulse assistant. Happy to walk you through the data for ${selectedNeighborhood.name} — what would you like to know?`
        : "Hey! I'm the Civic Pulse assistant. I can break down neighborhood data, Green Scores, and sustainability metrics — what are you curious about?";
  ...
  const resetChat = async () => {
    try {
      ...
      setTimeout(() => {
        const welcomeText = "Conversation cleared. What can I help you dig into next?";
